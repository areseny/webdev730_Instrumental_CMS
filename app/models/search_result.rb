class SearchResult < ActiveRecord::Base
  ...
  scope :search, -> (query) {
    where(%[(((setweight(to_tsvector('portuguese', f_unaccent(coalesce("search_results"."title"::text, ''))), 'A') || setweight(to_tsvector('portuguese', f_unaccent(coalesce("search_results"."content"::text, ''))), 'B')) @@ (to_tsquery('portuguese', ''' ' || f_unaccent(?) || ' '''))))], query)
    .order(%[((ts_rank((setweight(to_tsvector('portuguese', f_unaccent(coalesce("search_results"."title"::text, ''))), 'A') || setweight(to_tsvector('portuguese', f_unaccent(coalesce("search_results"."content"::text, ''))), 'B')), (to_tsquery('portuguese', ''' ' || f_unaccent(#{sanitize(query)}) || ' ''')), 0))) DESC])
  }
